...
export const getHeaders = (method, onlyAuth = false) => {
	let headers = {
		'Authorization': ENV.api.auth_header+" "+getAccessToken(store.getState()) || undefined
	};
	if (onlyAuth) {
		return headers;
	}
...
export const request = options => {
	...
	log('api', `${options.method.toUpperCase()} ${options.url} ${options.params ? JSON.stringify(options.params) : ''} ==>`, options.data);
